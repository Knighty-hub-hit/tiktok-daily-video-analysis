...
这一阶段的目标是让每日更新不依赖本机电脑。旧的 GitHub Actions 云端任务已经停用，只保留手动占位入口：

```text
.github/workflows/daily-refresh.yml
```

它不会再定时执行、不会再部署 GitHub Pages、也不会再向飞书群发送成功或失败通知。当前飞书群通知只应该来自本机 Codex 自动任务。

## 旧方案曾经接好的部分

云端任务曾经会做这些事；以下只作为以后迁移云服务器或重建云链路时的参考：
